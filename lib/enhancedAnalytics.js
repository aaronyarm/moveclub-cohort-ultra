...
export const processEnhancedAnalytics = (data) => {
  ...
  // Track unique customer+amount combinations to avoid counting retries
  const uniqueFailures = new Map();
  
  failedTransactions.forEach(row => {
    const customerId = row[cols.customerId] || row[cols.customer] || 'unknown';
    const amount = parseFloat(row[cols.amount] || 0);
    const reason = row[cols.declineReason] || 'unknown';
    const date = new Date(row[cols.date]);
    
    // Create unique key: customer + amount (to identify same payment retries)
    const uniqueKey = `${customerId}-${amount.toFixed(2)}`;
    
    // Only count the first failure for this customer+amount combination
    if (!uniqueFailures.has(uniqueKey)) {
      uniqueFailures.set(uniqueKey, { reason, amount, date });
  ...
  const declineReasonsArray = Object.entries(declineReasons).map(([reason, stats]) => ({
    ...
    percentage: ((stats.count / uniqueFailures.size) * 100).toFixed(1)
